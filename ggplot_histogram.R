...
#Basic, Ungrouped Histogram
ggplot(mydata, aes(x=G))+
  geom_histogram(color='black',fill='#336699', lwd=2)+
  theme_classic()+
  xlab('Goals')+
  ylab('Counts')+
...
ggplot(mydata, aes(x=G, fill=Pos))+
  geom_histogram(aes(y=..density..), lwd=2, color='black', alpha=0.8,
                 position = 'identity')+
  geom_density(alpha=0.2, color='black', lwd=1)+
  theme_classic()+
  xlab('Goals')+
  ylab('Density')+
  theme(
