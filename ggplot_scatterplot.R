#This script provides an example of a scatter plot created with ggplot2
...
#Basic, Ungrouped Scatter
ggplot(mydata, aes(x=TOI, y=G))+
  geom_point(color='#3399ff', size=1.5)+
  theme_classic()+
  xlab('Time On Ice (minutes)')+
  ylab('Goals')+
...
#Basic Grouped Scatter
  #Un-comment geom_smooth to add fit lines
ggplot(mydata, aes(x=TOI, y=G))+
  geom_point(aes(color=Pos), size=1.5)+
  #geom_smooth(aes(group=Pos),color='grey',lwd=1.5)+
  theme_classic()+
  xlab('Time On Ice (minutes)')+
  ylab('Goals')+
  guides(colour = guide_legend(override.aes = list(size=4)))+
  theme(
    ...
    legend.text = element_text(color='black', size = 20,  margin = margin(t = 0, r = 30, b = -10, l = 0))
  )
